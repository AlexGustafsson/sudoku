export default class Sudoku {
  constructor() {
    this.grid = Array.from({length: 9}).fill(null).map(() => {
      return Array.from({length: 9}).fill(null).map(() => {
        return {number: null, correct: false, legal: true};
      });
    });

    this.correctGrid = Array.from({length: 9}).fill(null).map(() => {
  ...
  log() {
    ...
    for (let index = 0; index < 81; index++) {
      ...
      string += this.getCorrectNumber(x, y) + ' ';
      if ((x + 1) % 3 === 0)
        string += '  ';
      if ((x + 1) % 9 === 0)
        string += '\n';
